...
def SPA_paper_comparison(n_outer_loops = 2):
    ...
    for idx_outer in range(outer_loops_to_skip, n_outer_loops):
        ...
        for idx_noise, var in enumerate(var_list):
            ...
            _, _, _, _, MSE_unbiased[idx_outer, idx_noise], _, _, _ = CV_mse('ALVEN', X_train, y_train_noisy, X_test, y_test,
                        cv_type = 'KFold', K_fold = 3, alpha = 20, l1_ratio = l1_ratio, degree = [4], trans_type = 'poly', ALVEN_cutoff = ALVEN_cutoff)
            # Biased model - only degree = 2
            _, _, _, _, MSE_biased[idx_outer, idx_noise], _, _, _ = CV_mse('ALVEN', X_train, y_train_noisy, X_test, y_test,
                        cv_type = 'KFold', K_fold = 3, alpha = 20, l1_ratio = l1_ratio, degree = [2], trans_type = 'poly', ALVEN_cutoff = ALVEN_cutoff)
            # CV model - degrees from 1 to 10
            hyperparams, _, _, _, MSE_cv[idx_outer, idx_noise], _, _, _ = CV_mse('ALVEN', X_train, y_train_noisy, X_test, y_test,
                        cv_type = 'KFold', K_fold = 3, alpha = 20, l1_ratio = l1_ratio, degree = list(range(1, 11)), trans_type = 'poly', ALVEN_cutoff = ALVEN_cutoff)
            selected_degree[idx_outer, idx_noise] = hyperparams['degree']
            # CV limited order model - degree 2 or 4
            _, _, _, _, MSE_cv_limited[idx_outer, idx_noise], _, _, _ = CV_mse('ALVEN', X_train, y_train_noisy, X_test, y_test,
                        cv_type = 'KFold', K_fold = 3, alpha = 20, l1_ratio = l1_ratio, degree = [2, 4], trans_type = 'poly', ALVEN_cutoff = ALVEN_cutoff)
    ...
    MSE_unbiased = np.median(MSE_unbiased, axis = 0)
    MSE_biased = np.median(MSE_biased, axis = 0)
    MSE_cv = np.median(MSE_cv, axis = 0)
    MSE_cv_limited = np.median(MSE_cv_limited, axis = 0)
    plt.rcParams.update({'lines.markersize': 4, 'font.size': 16})
    fig, ax = plt.subplots(dpi = 300)
    ax.plot(var_list, MSE_unbiased, 'o-', label = 'Unbiased Model')
    ax.plot(var_list, MSE_biased, 'o-', label = 'Biased Model')
    ax.plot(var_list, MSE_cv, 'o-', label = 'CV Model')
    ax.plot(var_list, MSE_cv_limited, 'o-', label = 'CV Limited Model')
    ...
    # Across the n_outer_loops, how often was a given degree picked when the noise was fixed?
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--n_outer_loops', type = int, nargs = 1, metavar = 2, default = [2], help = 'The number of outer loops to run (so that the final result is the median). The SPA paper used 3,000.')
