...
def CVpartition(X, y, Type = 'Re_KFold', K = 5, Nr = 1000, random_state = 0, group = None):
    """
...
def CV_mse(model_name, X, y, X_test, y_test, cv_type = 'Re_KFold', K_fold = 5, Nr = 1000, eps = 1e-4, alpha_num = 20, group = None, round_number = '', **kwargs):
    ...
    **kwargs : dict, optional
    ...
    ########################################################################################################################################   
        ...
        MSE_result = np.zeros( (len(kwargs['cell_type']), len(kwargs['activation']), len(kwargs['RNN_layers']), 1) )
        S = np.zeros( (len(kwargs['cell_type']), len(kwargs['activation']), len(kwargs['RNN_layers']), 1) )
            ...
            for i in range(len(kwargs['cell_type'])):
                for j in range(len(kwargs['activation'])):
                    for k in range(len(kwargs['RNN_layers'])):
                        ...
                        S[i, j, k, counter] = k+i+j
            
        MSE_mean = np.sum(MSE_result, axis = 4) / (counter+1) # TODO: clean this section
        MSE_std = np.std(MSE_result, axis = 4)
        ...
        print(f'Min MSE + 1 stdev: {MSE_bar}')
        # Find the one that right below/ith the smallest number of variables
        ...
        RNN_layers = kwargs['RNN_layers'][int(final_ind[2][0])]
